...
class PencairanFinalisasiExport implements FromArray, WithHeadings
{
    /**
    * @return \Illuminate\Support\Collection
    */

    protected $pencairans;

    public function __construct($pencairans = [])
    {
        $this->pencairans = $pencairans;
    }
    ...
    public function array(): array {
        ...
        foreach ($this->pencairans as $pencairan) {


            $mahasiswa = $pencairan->mahasiswa;

            $pendapatan = "-";

            // if (count($mahasiswa->evaluasi)) {
            //     $eval = $mahasiswa->evaluasi[0];
            //     if ($eval->pendapatan->max_pendapatan && $eval->pendapatan->min_pendapatan) {
            //         $pendapatan = number_format($eval->pendapatan->min_pendapatan) . ' - ' . number_format($eval->pendapatan->max_pendapatan);
            //     } elseif ($eval->pendapatan->max_pendapatan) {
            //         $pendapatan = '> ' . number_format($eval->pendapatan->max_pendapatan);
            //     } else {
            //         $pendapatan = '< ' . number_format($eval->pendapatan->min_pendapatan);
            //     }
            // }
            ...
            $rows[] = [
                $mahasiswa->pin, //PIN
                $mahasiswa->nim ?? '-', // NIM
                str_pad($pencairan->id, 4, '0', STR_PAD_LEFT), // No. Pencairan
                ...
                $mahasiswa->nisn ?? '-', // NISN
                $mahasiswa->gpa ?? '-', // GPA
                $mahasiswa->bank->name ?? '-', // Bank
                $mahasiswa->nama_rekening_bank ?? '-', // Nama Rekening
                $mahasiswa->no_rekening_bank ?? '-', // No. Rekening
